...
#pragma warning restore CA2208
    ...
    public Task<IList<T>> PaginateAll<T>(IPaginatable<T> firstPage, IPaginator? paginator = null, CancellationToken? cancellationToken = null)
    {
    ...
    public IEnumerable<Task<T>> PaginateConcurrently<T, TPaginatable>(TPaginatable firstPage, IPaginator? paginator = null, CancellationToken? cancellationToken = null)
      where TPaginatable : IPaginatable<T>, IFinitePaginatable
    {
      return (paginator ?? DefaultPaginator).PaginateConcurrently<T, TPaginatable>(firstPage, _apiConnector, cancellationToken);
    }
    ...
    /// <typeparam name="TPaginatable">Paging Container Type</typeparam>
    /// <returns>An IEnumerable containing a task for each expected item</returns>
    public IEnumerable<Task<T>> PaginateConcurrently<T, TNext, TPaginatable>(
    ...
    {
      return (paginator ?? DefaultPaginator).PaginateConcurrently<T, TNext, TPaginatable>(firstPage, mapper, _apiConnector, cancellationToken);
    }
